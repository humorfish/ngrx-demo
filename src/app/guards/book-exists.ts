...
import { filter, take, map, catchError, tap, switchMap } from 'rxjs/operators';
...
export class BookExistsGuard implements CanActivate {
	constructor(
		...
		private googleBooks: GoogleBookService,
	...
	hasBookInApi(id: string): Observable<boolean> {
		return this.googleBooks.retrieveBook(id)
			.pipe(
				...
				tap((action: book.LoadAction) => this.store.dispatch(action)),
				map(bookT => !!bookT),
				catchError(() => {
